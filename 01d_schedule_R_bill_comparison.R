...
# load data
monthly_data_currentBlock <- readRDS("D:/OneDrive - hawaii.edu/Documents/Projects/HECO/Data/Output/Residential/01c_schedule_R_bill_calculations_currentBlock.rds")
monthly_data_mcPricing <- readRDS("D:/OneDrive - hawaii.edu/Documents/Projects/HECO/Data/Output/Residential/01c_schedule_R_bill_calculations_marginalCharge.rds")
...
# merge data
monthly_data_currentBlock$RATE <- monthly_data_currentBlock$PV <- monthly_data_currentBlock$CENSUS_TRACT <- NULL
monthly_data <- merge(monthly_data_mcPricing, monthly_data_currentBlock,
                      by = c('ID', 'billing_year', 'billing_month'))
colnames(monthly_data)[colnames(monthly_data) == 'netValue_monthlyDollars'] <- 'bill_dollars_MC_NoFixedCharge'

# total revenue for each type
total_revenue_currentBlock <- sum(monthly_data$bill_dollars_tariff)
total_revenue_mcPricing <- sum(monthly_data$bill_dollars_MC_NoFixedCharge)
...
#fixed_monthly_charge*nrow(monthly_data) + sum(monthly_data$bill_dollars_MC_NoFixedCharge) == sum(monthly_data$bill_dollars_tariff)  # test equality of revenues

# add fixed cost to MC pricing to get total bill
monthly_data$monthlyFixedCharge <- fixed_monthly_charge
monthly_data$bill_dollars_MC_wFixedCharge <-
  monthly_data$bill_dollars_MC_NoFixedCharge + fixed_monthly_charge
...
monthly_data_median_bill_MC <- aggregate(monthly_data$bill_dollars_MC_wFixedCharge,
                                         list(monthly_data$ID, monthly_data$`PV status`),
                                         median)
colnames(monthly_data_median_bill_MC) <- c('ID', 'PV status', 'bill_dollars_MC_wFixedCharge_median')
...
monthly_data$billMC_minus_billBlock <-
  monthly_data$bill_dollars_MC_wFixedCharge - monthly_data$bill_dollars_tariff
monthly_data_median$billMC_minus_billBlock_median <-
  monthly_data_median$bill_dollars_MC_wFixedCharge_median -
...
aggregate(monthly_data_median$bill_dollars_tariff_median,
          list(monthly_data_median$`PV status`), summary)
aggregate(monthly_data_median$bill_dollars_MC_wFixedCharge_median,
          list(monthly_data_median$`PV status`), summary)
...
aggregate(monthly_data$bill_dollars_tariff,
          list(monthly_data$`PV status`), summary)
aggregate(monthly_data$bill_dollars_MC_wFixedCharge,
          list(monthly_data$`PV status`), summary)

# save data
saveRDS(monthly_data,
        file = 'D:/OneDrive - hawaii.edu/Documents/Projects/HECO/Data/Output/Residential/01d residential mc pricing.rds')
...
ggplot(monthly_data_median) + geom_histogram(aes(bill_dollars_MC_wFixedCharge_median,
                                                 color = `PV status`,
                                                 fill = `PV status`),
                                             alpha = 0.2) +
  xlim(-100, 500) +
...
ggplot(monthly_data_median) + geom_density(aes(bill_dollars_MC_wFixedCharge_median,
                                               color = `PV status`,
                                               fill = `PV status`),
                                           alpha = 0.2) +
  xlim(-100, 500) +
...
ggplot(monthly_data_median) +
  geom_histogram(aes(bill_dollars_MC_wFixedCharge_median - bill_dollars_tariff_median,
...
ggplot(monthly_data_median) +
  geom_density(aes(bill_dollars_MC_wFixedCharge_median - bill_dollars_tariff_median,
                   color = `PV status`, fill = `PV status`), alpha = 0.2) +
  xlim(-500,250) +
