...
z$year <- year(z$dt)
z$month <- month(z$dt)
monthly_consumption <- aggregate(z$KW_NET, list(z$ID, z$year, z$month), sum, na.rm = TRUE)
...
monthly_consumption <- monthly_consumption[order(monthly_consumption$ID, monthly_consumption$year, monthly_consumption$month), ]
